...
def main():
    ...
    import numpy as _np
    from sklearn.cluster import AgglomerativeClustering as _Agg, MiniBatchKMeans as _MBK
    _rng = _np.random.RandomState(42)
    _sub_n = best_params["subsample_n"]
    _idx = _rng.choice(len(X_sel), min(_sub_n, len(X_sel)), replace=False)
    _X_sub = X_sel[_idx]
    _agg = _Agg(n_clusters=best_params["n_clusters"], linkage="ward")
    _sub_labels = _agg.fit_predict(_X_sub)
    _centroids = _np.array([_X_sub[_sub_labels == l].mean(axis=0) for l in _np.unique(_sub_labels)])
    _km = _MBK(n_clusters=best_params["n_clusters"], init=_centroids, n_init=1, max_iter=300, random_state=42)
